...
def recolector_ingresos(request):
    ...
    lista = []
    for s in sql_1:
        if user == s.recolector.user.id:
            print(s.id)
            sql_2 = residuos.objects.all().select_related('usuarios_asociados__productor__user__comuna').order_by('-fecha')
            print (sql_2.query)
            for i in sql_2:
                if i.usuarios_asociados.id == s.id:
                    print(i.usuarios_asociados.productor.user.first_name)
                    print(i.usuarios_asociados.productor.user.comuna.name)
                    lista.append(i)
...
def recolector_rutas(request):
    ...
    for s in sql_1:
        if user == s.recolector.user.id:
            #print(s.id)
            sql_2 = asociados.objects.all().select_related('productor__user__comuna__region')
            #print (sql_2.query)
            for i in sql_2:
                if i.id == s.id:
                    print(i.productor.user.first_name)
                    print(i.productor.user.comuna.name)
                    print(i.productor.user.comuna.region.name)
                    lista.append(i)
